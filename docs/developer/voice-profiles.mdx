...
## Recording Lifecycle and Device Model

Voicebox recording surfaces now use a canonical lifecycle state machine:
...
Invalid state jumps are rejected by shared transition guards in frontend hooks.

System capture device data is normalized to:
...
This allows consistent behavior across webview and native/Tauri capture paths.

## Processing Stage Semantics

Recording/transcription processing is modeled with ordered stages:
...
Each processing operation carries a stable `task_id` (client-supplied via `x-task-id` when available) and is exposed through `/tasks/active` under `recording_processing`.

This supports:

- stage-aware progress rendering
- concurrent task separation
- restore-on-refresh task visibility
